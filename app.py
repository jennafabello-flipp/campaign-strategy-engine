...
def scrub_and_load_excel(file_obj, is_local_path=False):
    ...
    try:
        ...
        if is_csv:
            # Use Python's built-in csv module for the raw scan, not pandas' C parser —
            # the C parser locks in a field count from the first row(s) and throws
            # "Error tokenizing data" the moment a later row (e.g. the real data,
            # after a short preamble) has a different number of fields. csv.reader
            # tolerates ragged rows natively, which is exactly what a raw,
            # not-yet-header-detected export tends to look like.
            import csv as csv_module
            text = file_bytes.decode('utf-8', errors='replace')
            raw_rows = list(csv_module.reader(io.StringIO(text)))
            max_cols = max((len(r) for r in raw_rows), default=0)
            raw_rows_padded = [r + [None] * (max_cols - len(r)) for r in raw_rows]
            df_raw = pd.DataFrame(raw_rows_padded)
        else:
        ...
        if is_csv:
            try:
                df_clean = pd.read_csv(io.BytesIO(file_bytes), skiprows=best_row, low_memory=False)
            except pd.errors.ParserError:
                # A stray malformed row further down in the file — fall back to the
                # more tolerant python engine and skip only the offending rows,
                # rather than failing the whole file.
                df_clean = pd.read_csv(io.BytesIO(file_bytes), skiprows=best_row, engine='python', on_bad_lines='warn')
                st.warning("⚠️ One or more rows in this file had an unexpected number of fields and were skipped. Double-check the source export if data looks incomplete.")
        else:
            df_clean = pd.read_excel(io.BytesIO(file_bytes), skiprows=best_row)
...
def render_dvm_module_performance():
    ...
    if module_file:
        df_mod_result, m_mod, _ = scrub_and_load_excel(module_file)
        if df_mod_result is not None and not df_mod_result.empty:
            df_mod = df_mod_result
            ...
            st.error("⚠️ Could not read this file — it may not match the expected Hosted Module Reporting format. Please check the file and try again.")
            return
...
def render_social_format_efficiency():
    st.markdown("<div class='main-header'>📣 Module 6: Offers Unlimited Campaign Performance</div>", unsafe_allow_html=True)
...
pipeline_mode = st.sidebar.radio(
    ...
    [
        ...
        "📱 Module 5: DVM Module Performance",
        "📣 Module 6: Offers Unlimited Campaign Performance (WIP - DNU)"
    ]
)
...
elif "Offers Unlimited" in pipeline_mode or "Module 6" in pipeline_mode:
